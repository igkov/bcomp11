...
void sh1106_contrast(uint8_t contrast) {
	sendCommand(0x81); sendCommand(contrast); // 0x81,0x7f Ч Set Contrast Control (0x7f) Ч €ркость. 
}

void sh1106_init(uint8_t contrast, uint8_t inverse) {
	sendCommand(0xAE); 
	sendCommand(0xA8); sendCommand(0x3F); // 0xA8,0x3F Ч Set Multiplex Ratio (0x3F) Ч эта команда определ€ет, сколько строк будет использовано.
	sendCommand(0xD5);
	sendCommand(0x81); sendCommand(contrast); // 0x81,0x7f Ч Set Contrast Control (0x7f) Ч €ркость. 
	sendCommand(0xA0); // 0xA1 Ч Set Segment Re-map Ч развертка по горизонтали слева направо.
	sendCommand(0xC0); // 0xC0 Ч Set Common Output Scan Direction Ч развертка сверху вниз
	sendCommand(0xDA); sendCommand(0x12); // 0xDA,0x12 Ч Common Pads Hardware Configuration (Alternative Mode Set) Ч прогрессивна€ развертка. ћожно включить и чересстрочную.
	sendCommand(0xD3); sendCommand(0x00); // 0xD3,0x00 Ч Set Display Offset (0x00)
	sendCommand(0x40); // 0x40 Ч Set Display Start Line 0
	if (inverse) {
		sendCommand(0xA7);
	} else {
		sendCommand(0xA6);
	}
	sendCommand(0xAF); // 0xAF Ч Display ON
}
